...
def generate_image(image_config: dict, study_name: str, data_path: Path, temp_dir: Path) -> Path:
    ...
    if image_config['type'] == 'pairplot':
        ...
        text = image_config.get('text', 'Placeholder').replace('{{ study_name }}', study_name)
...
def generate_single_report(study_dir: Path, template_dir: Path, output_dir: Path):
    ...
    for key, image_config in config.get('images', {}).items():
        image_paths[key] = generate_image(image_config, study_dir.name, data_path, temp_image_dir)
