...
def setup_and_teardown_test_environment():
    ...
    cmd = [
        ...
        "--template-dir", str(TEST_PROJECT_DIR / "templates"),
        "--output-dir", str(GENERATED_REPORTS_DIR)
    ]
    subprocess.run(cmd, check=True, capture_output=True, text=True, cwd=TEST_PROJECT_DIR)
...
def test_report_file_is_generated(study_dir):
    ...
    assert expected_report_path.stat().st_size > 0, f"Report '{report_name}' is empty."

@pytest.mark.parametrize("study_dir", get_all_study_dirs())
def test_docx_content_matches_checkpoint(study_dir):
    """Tests that the text content of the .docx file matches the checkpoint for each study."""
    report_name = f"{study_dir.name}_report.docx"
